...
$(document).ready(function() {
	...
	var $el 			= $( '#card-ul' ),
		...
		if ( $(window).width() > 480) {
			sectionartists.appear(function(){
				baraja.fan({
					speed : 1500,
					easing : 'ease-out',
					range : 100,
	...
	// artists navigation
	$('#artists-prev').on( 'click', function( event ) {
		baraja.previous();
	});

	$('#artists-next').on( 'click', function( event ) {
		baraja.next();
	});
	
	// close artistss
	$('#artists-close').on( 'click', function( event ) {
		baraja.close();
	});	
});
